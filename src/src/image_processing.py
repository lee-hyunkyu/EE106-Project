...
def analyze(img):
    ...
    for row in range(len(immediate_front)):
        for col in range(len(immediate_front[row])):
            ...
            if col < 3*median_col/2 or row < median_row/2:
                right[row][col] = 0
            if row > median_row/2 or abs(median_col - col) > median_col/3 or row < median_row/4:
                forward[row][col] = 0
    ...
    forward = sum(sum(forward)) > 7000


    # print((median - median_row), left, right, forward)
    # if len(locations) != 0:
    #     # print(lo)
    #     med_row = np.median(locations[0])
    #     locations = [c for r, c in zip(locations[0], locations[1]) if r < med_row]
    #     median = sum(locations)/len(locations)
    # else:
    #     median = 0
    # median_diff = (median - median_row) - 60
    # print(median_diff)
    # cv2.imshow('img', immediate_front)
    # cv2.waitKey(1)
